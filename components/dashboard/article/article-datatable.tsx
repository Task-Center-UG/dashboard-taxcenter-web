...
import { columns, ArticleColumn } from "./article-columns";

interface ArticleResponse {
  articles: ArticleColumn[];
  paging: {
    page: number;
    total_pages: number;
    total_items: number;
...
export default function ArticleDataTable() {
  ...
  const { data, isLoading } = useGetData<ArticleResponse>({
    key: [
      "article-list",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data artikel pajak.
          </p>
        </div>
        <Button
          onClick={() => router.push("/dashboard/activities/articles/create")}
