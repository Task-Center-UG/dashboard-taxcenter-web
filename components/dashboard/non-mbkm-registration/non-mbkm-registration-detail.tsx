...
interface NonMbkmRegistrationDetailData {
  id: number;
  full_name: string;
  class: string;
  npm: string;
...
export default function NonMbkmRegistrationDetail({ id }: { id: string }) {
  ...
  const { data, isLoading } = useGetData<NonMbkmRegistrationDetailData>({
    key: ["non-mbkm-registration-detail", id],
    url: `/tax-volunteer/non-mbkm-registration/${id}`,
  });
  ...
  const { mutate: updateData, isPending: isUpdating } = usePutData({
    ...
    invalidateKeys: [
      ["non-mbkm-registrations-list"],
      ["non-mbkm-registration-detail", id],
    ],
    successMessage: "Status relawan berhasil diperbarui",
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["non-mbkm-registration-detail", id],
        });
      },
    },
  });
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex items-center gap-4">
        <Button
          ...
          onClick={() => router.push("/dashboard/tax-volunteers/non-mbkm")}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent className="pt-4">
            ...
            <div className="mt-4 pt-4 border-t border-zinc-100 dark:border-zinc-800">
              <div className="mb-2 text-sm font-medium text-muted-foreground flex items-center gap-2">
                <ShieldCheck className="h-4 w-4" /> Kegiatan
              </div>
        ...
        <Card className="shadow-sm border-zinc-200 dark:border-zinc-800 md:col-span-2">
          ...
          <CardContent className="pt-4 grid sm:grid-cols-3 gap-4">
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 p-4 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-pink-100 dark:bg-pink-900/20 text-pink-600 rounded-lg flex items-center justify-center">
                  <ImageIcon className="h-5 w-5" />
                </div>
                <div>
                  <p className="font-semibold text-sm">Bukti Follow</p>
                  <p className="text-xs text-muted-foreground">Screenshot IG</p>
                </div>
              </div>
              {data.screenshot ? (
                <Button variant="outline" size="sm" asChild>
                  <a
                    href={getImageUrl(data.screenshot)}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Lihat
