...
interface AgendaSliderCellActionProps {
  data: {
    id: number;
  };
}

export function AgendaSliderCellAction({ data }: AgendaSliderCellActionProps) {
  ...
  const { mutate: deleteItem, isPending: isDeleting } = useDeleteData({
    ...
    successMessage: `Slider #${data.id} berhasil dihapus`,
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["agenda-slider-list"],
          type: "active",
        });
        setDeleteOpen(false);
      },
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="rounded-xl">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/activities/sliders/${data.id}/update`)
            }
