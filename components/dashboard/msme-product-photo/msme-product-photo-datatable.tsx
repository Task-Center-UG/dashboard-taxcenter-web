...
import { columns, MsmeProductPhotoColumn } from "./msme-product-photo-columns";

interface MsmeProductPhotoResponse {
  umkmProductPhoto: MsmeProductPhotoColumn[];
  paging: {
    page: number;
    total_pages: number;
    total_items: number;
...
export default function MsmeProductPhotoDataTable() {
  ...
  const { data, isLoading } = useGetData<MsmeProductPhotoResponse>({
    key: [
      "umkm-product-photo-list",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data foto produk UMKM.
          </p>
        </div>
        <Button
          onClick={() => router.push("/dashboard/service/msme-product-photos/create")}
