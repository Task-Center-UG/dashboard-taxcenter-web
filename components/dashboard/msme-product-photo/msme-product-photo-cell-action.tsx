...
interface MsmeProductPhotoCellActionProps {
  data: {
    id: number;
    title: string;
  };
...
export function MsmeProductPhotoCellAction({ data }: MsmeProductPhotoCellActionProps) {
  ...
  const { mutate: deleteItem, isPending: isDeleting } = useDeleteData({
    ...
    invalidateKeys: [["umkm-product-photo-list"]],
    successMessage: `"${data.title}" berhasil dihapus`,
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["umkm-product-photo-list"],
          type: "active",
        });
        setDeleteOpen(false);
      },
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="rounded-xl">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/service/msme-product-photos/${data.id}/update`)
            }
