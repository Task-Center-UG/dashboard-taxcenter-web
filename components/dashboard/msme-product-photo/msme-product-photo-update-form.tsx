...
interface MsmeProductPhotoDetailData {
  id: number;
  title: string;
  description: string;
  image_url: string;
...
export default function MsmeProductPhotoUpdateForm({ id }: { id: string }) {
  ...
  const { data: detailData, isLoading: isLoadingDetail } =
    useGetData<MsmeProductPhotoDetailData>({
      key: ["umkm-product-photo-detail", id],
  ...
  const mutation = usePutData<any, FormData>({
    ...
    invalidateKeys: [
      ...
      ["umkm-product-photo-detail", id],
    ],
    successMessage: "Data berhasil diperbarui",
    options: {
      onSuccess: () => {
        router.push("/dashboard/service/msme-product-photos");
      },
    },
  });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Foto Produk UMKM</CardTitle>
        <CardDescription>
          Perbarui data foto produk UMKM di bawah ini.
        </CardDescription>
      </CardHeader>
