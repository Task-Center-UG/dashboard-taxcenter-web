...
interface MsmeAssistanceCellActionProps {
  data: {
    id: number;
    title: string;
  };
...
export function MsmeAssistanceCellAction({ data }: MsmeAssistanceCellActionProps) {
  ...
  const { mutate: deleteItem, isPending: isDeleting } = useDeleteData({
    url: "/msme-assistance",
    invalidateKeys: [["msme-assistance-list"]],
    successMessage: `"${data.title}" berhasil dihapus`,
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["msme-assistance-list"],
          type: "active",
        });
        setDeleteOpen(false);
      },
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="rounded-xl">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/service/msme-mentoring/${data.id}/update`)
            }
