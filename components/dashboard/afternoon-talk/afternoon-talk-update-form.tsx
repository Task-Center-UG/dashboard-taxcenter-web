...
interface AfternoonTalkDetailData {
  id: number;
  title: string;
  description: string;
  image_url: string;
...
export default function AfternoonTalkUpdateForm({ id }: { id: string }) {
  ...
  const { data: detailData, isLoading: isLoadingDetail } =
    useGetData<AfternoonTalkDetailData>({
  ...
  const mutation = usePutData<any, FormData>({
    url: "/afternoon-talk",
    invalidateKeys: [["afternoon-talk-list"], ["afternoon-talk-detail", id]],
    successMessage: "Bincang sore berhasil diperbarui",
    options: {
      onSuccess: () => {
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Bincang Sore</CardTitle>
        <CardDescription>
          Perbarui data bincang sore di bawah ini.
        </CardDescription>
      </CardHeader>
