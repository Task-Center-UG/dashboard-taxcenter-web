...
import { Loader2, Save, Type, UploadCloud, ImageIcon, Video } from "lucide-react";
...
const createFormSchema = z.object({
  ...
  video_url: z.string().min(1, "URL video wajib diisi").url("URL video tidak valid"),
  image_url: z
...
export default function AfternoonTalkCreateForm() {
  ...
  const mutation = usePostData<any, FormData>({
    url: "/afternoon-talk",
    invalidateKeys: [["afternoon-talk-list"]],
    successMessage: "Bincang sore berhasil ditambahkan",
    options: {
      onSuccess: () => {
        router.push("/dashboard/tax-education/afternoon-talks");
      },
    },
  });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Tambah Bincang Sore</CardTitle>
        <CardDescription>
          Isi formulir di bawah untuk menambahkan bincang sore baru.
        </CardDescription>
      </CardHeader>
