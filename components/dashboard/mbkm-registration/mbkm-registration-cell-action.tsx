...
interface MbkmRegistrationCellActionProps {
  data: {
    id: number;
    full_name: string;
  };
...
export function MbkmRegistrationCellAction({
  data,
}: MbkmRegistrationCellActionProps) {
  ...
  const { mutate: deleteItem, isPending: isDeleting } = useDeleteData({
    ...
    invalidateKeys: [["mbkm-registrations-list"]],
    successMessage: `Pendaftaran "${data.full_name}" berhasil dihapus`,
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["mbkm-registrations-list"],
          type: "active",
        });
        setDeleteOpen(false);
      },
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="rounded-xl">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/tax-volunteers/mbkm/${data.id}`)
            }
