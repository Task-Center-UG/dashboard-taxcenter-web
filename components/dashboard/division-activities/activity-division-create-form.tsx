...
export default function ActivityDivisionCreateForm({
  divisionId,
}: {
  divisionId: string;
}) {
  ...
  const mutation = usePostData<any, FormData>({
    ...
    successMessage: "Kegiatan divisi berhasil dibuat",
    options: {
      onSuccess: () => {
        router.push(
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Tambah Kegiatan Divisi</CardTitle>
        <CardDescription>
          Isi formulir di bawah ini untuk menambahkan kegiatan baru.
        </CardDescription>
      </CardHeader>
