...
interface TaxMaterialDetailData {
  id: number;
  title: string;
  description: string;
  image_url: string;
...
export default function TaxMaterialUpdateForm({ id }: { id: string }) {
  ...
  const { data: detailData, isLoading: isLoadingDetail } =
    useGetData<TaxMaterialDetailData>({
  ...
  const mutation = usePutData<any, FormData>({
    url: "/tax-material",
    invalidateKeys: [["tax-material-list"], ["tax-material-detail", id]],
    successMessage: "Materi pajak berhasil diperbarui",
    options: {
      onSuccess: () => {
        router.push("/dashboard/tax-education/materials");
      },
    },
  });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Materi Pajak</CardTitle>
        <CardDescription>
          Perbarui data materi pajak di bawah ini.
        </CardDescription>
      </CardHeader>
              ...
              <div className="p-6 lg:p-8 lg:col-span-2 space-y-6 bg-white dark:bg-zinc-950 min-w-0">
                <div className="grid gap-6">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>URL File (Google Drive)</FormLabel>
                        <FormControl>
                          <div className="relative">
