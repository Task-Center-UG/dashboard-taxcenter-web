...
import { columns, SeminarColumn } from "./seminar-columns";

interface SeminarResponse {
  seminars: SeminarColumn[];
  paging: {
    page: number;
    total_pages: number;
    total_items: number;
...
export default function SeminarDataTable() {
  ...
  const { data, isLoading } = useGetData<SeminarResponse>({
    key: [
      "seminar-list",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button
          onClick={() => router.push("/dashboard/activities/seminars/create")}
