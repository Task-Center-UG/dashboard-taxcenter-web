...
interface TaxLearningVideoDetailData {
  id: number;
  title: string;
  description: string;
  image_url: string;
...
export default function TaxLearningVideoUpdateForm({ id }: { id: string }) {
  ...
  const { data: detailData, isLoading: isLoadingDetail } =
    useGetData<TaxLearningVideoDetailData>({
  ...
  const mutation = usePutData<any, FormData>({
    ...
    invalidateKeys: [
      ...
      ["tax-learning-video-detail", id],
    ],
    successMessage: "Video pembelajaran pajak berhasil diperbarui",
    options: {
      onSuccess: () => {
        router.push("/dashboard/tax-education/learning-videos");
      },
    },
  });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Video Pembelajaran Pajak</CardTitle>
        <CardDescription>
          Perbarui data video pembelajaran pajak di bawah ini.
        </CardDescription>
      </CardHeader>
