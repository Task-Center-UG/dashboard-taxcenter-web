...
const updateFormSchema = z.object({
  ...
  cta_url: z.string().url("Format URL tidak valid"),
...
export default function SliderUpdateForm({ id }: { id: string }) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(null);

  const { data: sliderData, isLoading: isLoadingSlider } =
    useGetData<SliderDetail>({
      key: ["slider-detail", id],
      url: `/sliders/${id}`,
    });
  ...
  useEffect(() => {
    if (sliderData) {
      form.reset({
        ...
        cta_url: sliderData.cta_url,
        description: sliderData.description,
      });
      setPreviewImage(sliderData.picture_url);
    }
  }, [sliderData, form]);

  const mutation = usePatchData<any, FormData>({
    url: "/sliders",
    invalidateKeys: [["sliders-list"], ["slider-detail", id]],
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        ...
        <CardDescription>Perbarui informasi slider/banner.</CardDescription>
      </CardHeader>
              ...
              <div className="p-8 md:w-2/3 space-y-6 bg-white dark:bg-zinc-950">
                <div className="grid gap-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Judul Slider</FormLabel>
                        <FormControl>
