...
import { columns, FgdColumn } from "./fgd-columns";

interface FgdResponse {
  fgds: FgdColumn[];
  paging: {
    page: number;
    total_pages: number;
...
export default function FgdDataTable() {
  ...
  const { data, isLoading } = useGetData<FgdResponse>({
    key: [
      "fgd-list",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data Focus Group Discussion (FGD).
          </p>
        </div>
        <Button
          onClick={() => router.push("/dashboard/activities/fgd/create")}
