...
export default function AwardUpdateForm({ id }: { id: string }) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [previewImage, setPreviewImage] = useState<string | null>(null);

  const { data: awardData, isLoading: isLoadingAward } =
    useGetData<AwardDetail>({
      key: ["award-detail", id],
      url: `/awards/${id}`,
    });
  ...
  useEffect(() => {
    if (awardData) {
      ...
      setPreviewImage(awardData.picture_url);
    }
  }, [awardData, form]);

  const mutation = usePatchData<any, FormData>({
    url: "/awards",
    invalidateKeys: [["awards-list"], ["award-detail", id]],
    successMessage: "Penghargaan berhasil diperbarui",
    options: {
      onSuccess: () => {
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Penghargaan</CardTitle>
        <CardDescription>Perbarui informasi penghargaan.</CardDescription>
      </CardHeader>
