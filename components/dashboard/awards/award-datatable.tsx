...
import { columns, AwardColumn } from "./award-columns";
...
export default function AwardDataTable() {
  ...
  const { data, isLoading } = useGetData<AwardsResponse>({
    key: [
      "awards-list",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data penghargaan yang diraih oleh Tax Center.
          </p>
        </div>
        <Button
          onClick={() => router.push("/dashboard/home/awwards/create")}
          className="bg-[#2D1B69] hover:bg-[#1e1247] text-white rounded-xl shadow-lg shadow-[#2D1B69]/20"
        >
          <Plus className="mr-2 h-4 w-4" /> Tambah Penghargaan
        </Button>
      </div>
