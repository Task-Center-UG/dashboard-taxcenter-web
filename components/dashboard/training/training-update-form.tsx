...
export default function TrainingUpdateForm({ id }: { id: string }) {
  ...
  const { data: detailData, isLoading: isLoadingDetail } =
    useGetData<TrainingDetailData>({
      key: ["training-detail", id],
      url: `/training/${id}`,
    });
  ...
  const mutation = usePutData<any, FormData>({
    url: "/training",
    invalidateKeys: [["training-list"], ["training-detail", id]],
    successMessage: "Data berhasil diperbarui",
    options: {
      onSuccess: () => {
        router.push("/dashboard/service/training");
      },
    },
  });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        ...
        <CardDescription>Perbarui data pelatihan di bawah ini.</CardDescription>
      </CardHeader>
