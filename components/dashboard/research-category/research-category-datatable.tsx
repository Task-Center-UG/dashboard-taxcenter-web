...
import { columns, ResearchCategoryColumn } from "./research-category-columns";

interface ResearchCategoryResponse {
  researchCategory: ResearchCategoryColumn[];
  paging: {
    page: number;
    total_pages: number;
    total_items: number;
...
export default function ResearchCategoryDataTable() {
  ...
  const { data, isLoading } = useGetData<ResearchCategoryResponse>({
    key: [
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data kategori penelitian.
          </p>
        </div>
        <Button
          onClick={() => router.push("/dashboard/research/research-categories/create")}
