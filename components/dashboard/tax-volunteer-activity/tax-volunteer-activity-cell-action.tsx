...
interface TaxVolunteerActivityCellActionProps {
  data: {
    id: number;
    title: string;
  };
...
export function TaxVolunteerActivityCellAction({ data }: TaxVolunteerActivityCellActionProps) {
  ...
  const { mutate: deleteItem, isPending: isDeleting } = useDeleteData({
    ...
    invalidateKeys: [["tax-volunteer-activity-list"]],
    successMessage: `"${data.title}" berhasil dihapus`,
    options: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ["tax-volunteer-activity-list"],
          type: "active",
        });
        setDeleteOpen(false);
      },
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="rounded-xl">
          ...
          <DropdownMenuItem
            onClick={() =>
              router.push(`/dashboard/tax-volunteers/activities/${data.id}/update`)
            }
