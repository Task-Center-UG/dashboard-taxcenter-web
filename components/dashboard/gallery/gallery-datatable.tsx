...
import { columns, GalleryColumn } from "./gallery-columns";

interface GalleryResponse {
  gallerys: GalleryColumn[];
  paging: {
    page: number;
    total_pages: number;
    total_items: number;
...
export default function GalleryDataTable() {
  ...
  const { data, isLoading } = useGetData<GalleryResponse>({
    key: [
      "gallery-list",
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Kelola data galeri foto kegiatan.
          </p>
        </div>
        <Button
