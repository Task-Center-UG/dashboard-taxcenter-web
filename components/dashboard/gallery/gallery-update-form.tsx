...
const updateFormSchema = z.object({
  ...
  picture_url: z.union([z.instanceof(File), z.string()]).optional(),
});
...
export default function GalleryUpdateForm({ id }: { id: string }) {
  ...
  const mutation = usePutData<any, FormData>({
    url: "/gallery",
    invalidateKeys: [["gallery-list"], ["gallery-detail", id]],
  ...
  const onSubmit = (values: UpdateFormValues) => {
    ...
    if (values.picture_url instanceof File) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      form.setValue("picture_url", file, { shouldValidate: true });
  ...
  return (
    <Card className="max-w-7xl mx-auto border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden">
      <CardHeader className="bg-zinc-50/50 dark:bg-zinc-900/50 border-b border-zinc-100 dark:border-zinc-800">
        <CardTitle>Edit Galeri</CardTitle>
        <CardDescription>
          Perbarui data galeri di bawah ini.
        </CardDescription>
      </CardHeader>
